function ExperienceCard({ title, plan, caracteristicas, solicitado, precio, precioOriginal, className = '' }) {
        ...
        <div className={`relative group ${solicitado ? 'bg-gradient-to-br from-cyan-600 to bg-purple-600' : 'border-2 border-purple-600'} w-full p-8 flex flex-col justify-between min-h-[600px] rounded-2xl transition-all duration-300 hover:-translate-y-2 ${className}`}>
            ...
            <div className="w-full text-center space-y-3 mb-6">
                ...
                <div className={`text-sm flex flex-col items-center gap-1 ${solicitado ? 'text-white/90' : 'text-gray-600'}`}>
                    {precioOriginal && precio !== "Contáctanos" && (
                        <div className="flex items-center gap-2">
                            <span className={`text-lg line-through opacity-60 ${solicitado ? 'text-white/70' : 'text-gray-500'}`}>
                                {precioOriginal}
                            </span>
                            <span className="px-2 py-1 bg-red-500 text-white text-xs font-bold rounded-full">-20%</span>
                        </div>
                    )}
                    <span className={`font-bold text-4xl ${solicitado ? 'text-white' : 'text-gray-900'}`}>
                        {precio}<span className="text-xs">MXN</span>
                    </span>
                    {precio !== "Contáctanos" && precioOriginal && (
                        <span className="text-xs opacity-70 mt-1 font-semibold">🎉 Oferta válida hasta el 31 de diciembre</span>
                    )}
                    {precio !== "Contáctanos" && !precioOriginal && <span className="text-xs opacity-70">Precio inicial</span>}
                </div>
            ...
            <ul className="space-y-4 mb-8 flex-grow">
                {caracteristicas?.map((caracteristica, index) => (
                    <li key={index} className="flex items-start gap-3 group/item">
                        <div className={`mt-0.5 w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${solicitado
            ...
            <div className="justify-center flex mt-auto">
                <a
                    href="#contact"
                    className={`group/btn w-full py-4 px-6 rounded-2xl font-bold text-center transition-all duration-300 hover:scale-105 flex items-center justify-center gap-2 ${solicitado
